...
use crate::domain::companies::{
    AttachCompanyLogoInput, CompanyInput, CompanyListQuery, archive_company, attach_company_logo,
    clear_company_logo, create_company, get_company, list_companies, resolve_company_logo_path,
    set_default_company, unarchive_company, update_company,
};
...
fn create_list_get_update_archive_flow() {
    ...
    let archived = archive_company(db.conn(), created.id).expect("archive");
    ...
    let restored = unarchive_company(db.conn(), created.id).expect("unarchive");
    assert!(restored.archived_at.is_none());
    assert!(!restored.is_default);

    let _ = get_company(db.conn(), created.id).expect("get");
}
...
fn set_default_clears_previous_in_one_transaction() {
    ...
    let updated = set_default_company(db.conn(), second.id).expect("set default");
    assert!(updated.is_default);
    assert!(!get_company(db.conn(), first.id).expect("first").is_default);
...
fn archive_default_clears_is_default() {
    ...
    let archived = archive_company(db.conn(), company.id).expect("archive");
    assert!(archived.archived_at.is_some());
    assert!(!archived.is_default);
}
...
fn attach_and_resolve_logo() {
    ...
    assert!(rel.starts_with(&format!("images/companies/{}/", company.id)));

    let resolved = resolve_company_logo_path(&db, company.id).expect("resolve");
    assert!(std::path::Path::new(&resolved.absolute_path).is_file());

    let cleared = clear_company_logo(&db, company.id).expect("clear");
    assert!(cleared.logo_path.is_none());

    let err = resolve_company_logo_path(&db, company.id).expect_err("missing");
