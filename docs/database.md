...
- **Migration 012** adds nullable `estimate_list_cents` and `estimate_discount_bps` on `repairs` (list price + discount at intake or diagnosis; same list→net→tax semantics as `create_repair`).
- **Migration 013** adds nullable `warranty_years` on `repairs` (set at summary handover; integer years `0..=10`).
